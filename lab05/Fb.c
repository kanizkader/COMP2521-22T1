...
bool FbUnfriend(Fb fb, char *name1, char *name2) {
    ...
    int id1 = nameToId(fb, name1);
    int id2 = nameToId(fb, name2);
    // ensure they aren't the same id/ same person
    assert(id1 != id2);

    if (fb->friends[id1][id2]) {
        fb->friends[id1][id2] = false;
        fb->friends[id2][id2] = false;
        return true;
    }
    return false;
}
...
void FbFriendRecs1(Fb fb, char *name) {
    // TODO: Complete this function
    int id1 = nameToId(fb, name);
    int *recs = calloc(fb->numPeople,sizeof(int));
    for (int i = 0; i < fb->numPeople; i++) {
        int count = 0;
        for (int j = 0; j < fb->numPeople; i++) {
            if (fb->friends[id1][j] && fb->friends[i][j]) {
                count++;
            } 
        }
        recs[i] = count;
    ...
    for (int k = fb->numPeople - 2; k > 0; k--) {
        for (int id = 0; id < fb->numPeople; id++) {
            if (recs[id] == k && !fb->friends[id1] [id]) {
