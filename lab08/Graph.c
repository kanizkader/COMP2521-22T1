...
Graph GraphMST(Graph g) {
    ...
    PQ sorted = PQNew();
    // traverse through graph and get all weights
    for (int v = 0; v < g->nV; v++) { 
        for (int w = 0; w < g->nV; w++) { 
            // if == 0.0, then edge does not exist
            if (g->edges[v][w] == 0.0) {
                break;
            }
            // else adds the edge to the list
            Edge newEdge;
            newEdge.v = v;
            newEdge.w = w;
            newEdge.weight = g->edges[v][w];
            PQInsert(sorted, newEdge); // insert into list
        }
    }
    Edge extract = PQExtract(sorted)
    // create MST using Kruskal
    for (Edge smallest = extract; !PQIsEmpty(sorted); smallest = extract) {
        // add edge to MST
        GraphInsertEdge(MST, smallest);
        // MST->nE++;
        // if MST has a cyle then drop edge
        if (GraphHasCycle(MST) == true) {
            GraphRemoveEdge(MST, smallest.v, smallest.w);
            // MST->nE--;
        }
        // if MST has n-1 edges then return MST
        if (MST->nE == (g->nV - 1)) {
            PQFree(sorted);
            return MST;
        }        
